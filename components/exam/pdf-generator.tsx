...
// New component that handles group selection + PDF generation
interface AnswerSheetExporterProps {
  exam: Exam;
  groups: Group[];
  paperSize: 'LETTER' | 'A4';
  onGenerated?: () => void;
...
export function AnswerSheetExporter({ 
  exam, 
  groups, 
  paperSize, 
  onGenerated, 
  ...
  const sortedStudents = useMemo(() => {
    ...
    return [...selectedGroup.estudiantes].sort((a, b) => 
      a.apellidos.localeCompare(b.apellidos, 'es', { sensitivity: 'base' })
    );
  }, [selectedGroup]);

  // Cache key for this exam+group combo
  const cacheKey = selectedGroupId ? getCacheKey(exam.id, selectedGroupId) : '';

  // Generate PDF function
  const generatePDF = useCallback(async (groupToGenerate: Group) => {
    const key = getCacheKey(exam.id, groupToGenerate.id);
    ...
    // Note: setIsGenerating(true) is called by handleGroupChange/useEffect BEFORE this
    setError(null);

    try {
      const students = [...groupToGenerate.estudiantes].sort((a, b) => 
        a.apellidos.localeCompare(b.apellidos, 'es', { sensitivity: 'base' })
      );
      ...
      const qrDataPromises = students.map(async (student) => {
      ...
      const styles = StyleSheet.create({
        page: { flexDirection: 'column', backgroundColor: '#ffffff', padding: 0, height: '100%' },
        container: { margin: markerDimensions.margin * 2, minHeight: 720, position: 'relative', display: 'flex', flexDirection: 'column' },
        headerSection: { padding: '5 10', height: 120, marginBottom: 8 },
        mainTitle: { fontSize: 16, textAlign: 'center', fontWeight: 'bold', marginBottom: 3 },
        infoContainer: { flexDirection: 'row', justifyContent: 'space-between', height: 100, paddingTop: 0 },
        qrCode: { width: 100, height: 100, marginTop: -8, marginLeft: -5 },
        studentInfo: { width: '65%', border: '3pt solid black', padding: 8 },
        infoTitle: { fontSize: 10, fontWeight: 'bold', marginBottom: 4 },
        infoText: { fontSize: 8, marginBottom: 3, lineHeight: 1.2 },
        answersSection: { padding: 10, position: 'relative', flexGrow: 1, border: '3pt solid black', margin: '8 8 15 8' },
        columnsContainer: { flexDirection: 'row', justifyContent: 'space-between', paddingVertical: 5, flexGrow: 1 },
        column: { width: '48%', borderRight: '1.5pt solid #cccccc', paddingRight: 8 },
        questionRow: { flexDirection: 'row', marginBottom: 5, alignItems: 'center', minHeight: 16, borderBottom: '1pt solid #cccccc', paddingBottom: 2 },
        questionNumberContainer: { width: 18, height: 18, marginRight: 8, backgroundColor: '#ffffff', display: 'flex', alignItems: 'center', justifyContent: 'center' },
        questionNumber: { fontSize: 8, fontWeight: 'bold', textAlign: 'center' },
        optionsRow: { flexDirection: 'row', gap: 16, flex: 1, paddingLeft: 2 },
        optionBubble: { width: 18, height: 18, position: 'relative', backgroundColor: '#ffffff' },
        instructions: { fontSize: 8, fontStyle: 'italic', textAlign: 'center', paddingHorizontal: 20, position: 'absolute', bottom: markerDimensions.margin + 5, left: markerDimensions.margin * 2, right: markerDimensions.margin * 2, color: '#000000' },
      });
      ...
      const getQuestionOptions = (pregunta: Exam['preguntas'][0]): string[] => {
        if (pregunta.opciones_respuesta.length === 2 && 
          ...
          if (firstOptionText === 'v' || firstOptionText.includes('verdadero')) return ['V', 'F'];
          return ['F', 'V'];
        }
      ...
      const CornerMarker = ({ position }: { position: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right' }) => {
        ...
        return <View style={containerStyle}><Svg width={size} height={size}><Path d={path} fill="#000000" /></Svg></View>;
      };
      ...
      const OptionBubble = ({ letter }: { letter: string }) => (
        <View style={styles.optionBubble}>
          ...
          <Text style={{ position: 'absolute', top: 4, left: letter.length > 1 ? 4 : 6, fontSize: 8, fontWeight: 'bold' }}>{letter}</Text>
        </View>
      );
      ...
      const QuestionRow = ({ pregunta, number }: { pregunta: Exam['preguntas'][0]; number: number }) => {
        ...
        return (
          <View style={styles.questionRow}>
            <View style={styles.questionNumberContainer}><Text style={styles.questionNumber}>{number}</Text></View>
            <View style={styles.optionsRow}>{options.map((letter, index) => <OptionBubble key={index} letter={letter} />)}</View>
          </View>
        );
      };
      ...
      const QuestionsColumn = ({ preguntas, startIndex }: { preguntas: Exam['preguntas']; startIndex: number }) => (
        <View style={styles.column}>
          {preguntas.map((pregunta, index) => <QuestionRow key={pregunta.id} pregunta={pregunta} number={startIndex + index + 1} />)}
        </View>
      );
                  ...
                  <View style={styles.container}>
                    <View style={styles.headerSection}>
                      ...
                      <View style={styles.infoContainer}>
                        ...
                        <View style={styles.studentInfo}>
                          ...
                          <Text style={styles.infoText}>{labels.group} {groupToGenerate.nombre}</Text>
                          <Text style={styles.infoText}>{labels.subject} {groupToGenerate.materia.nombre}</Text>
                    ...
                    <View style={styles.answersSection}>
                      <View style={styles.columnsContainer}>
                        ...
                        {preguntasCol2.length > 0 && <QuestionsColumn preguntas={preguntasCol2} startIndex={pageIndex * 40 + 20} />}
                      </View>
                    </View>
                  </View>
      ...
      const PDFDocument = (
        <Document>
          {students.map((student) => (
            <AnswerSheet key={student.id} student={student} qrDataUrl={qrMap.get(student.id) || ''} />
          ))}
        </Document>
      );
  ...
  useEffect(() => {
    ...
    // Only auto-generate once per group
    if (!hasAutoGenerated.current.has(cacheKey)) {
      hasAutoGenerated.current.add(cacheKey);
      // Set generating state IMMEDIATELY before async call
      setIsGenerating(true);
      setPdfUrl(null);
      generatePDF(selectedGroup);
    }
  }, [selectedGroup, cacheKey, generatePDF]);

  // Handle group change - set generating state immediately for instant feedback
  const handleGroupChange = (groupId: string) => {
    const cached = pdfCache.get(getCacheKey(exam.id, groupId));
    if (!cached) {
      setIsGenerating(true);
      setPdfUrl(null);
    }
  ...
  const handleDownload = useCallback(() => {
    if (pdfUrl && selectedGroup) {
      const link = document.createElement('a');
      link.href = pdfUrl;
      link.download = `hojas-respuesta-${exam.titulo}-${selectedGroup.nombre}.pdf`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      onGenerated?.();
    }
  }, [pdfUrl, selectedGroup, exam.titulo, onGenerated]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <Label className="text-sm font-semibold">{selectGroupLabel}</Label>
        <Select value={selectedGroupId} onValueChange={handleGroupChange}>
          <SelectTrigger className="w-full max-w-xs">
            <SelectValue placeholder={selectGroupPlaceholder} />
          </SelectTrigger>
          <SelectContent>
            {groups.map((g) => (
...
export function PDFGenerator({ exam, group, paperSize, fileName, onGenerated, labels }: PDFGeneratorProps) {
  ...
  // Show download button (disabled while generating)
  return (
    <Button 
      onClick={handleDownload} 
      disabled={!pdfUrl || isGenerating}
