  ...
  try {
    ...
    if (!supabaseUrl || !supabaseServiceKey) {
      return NextResponse.json(
        {
          error: (await getApiTranslator(request, "dashboard")).t(
            "errors.serverConfig"
          ),
        },
        { status: 500 }
    ...
    const [
      ...
      // 3. Grupos por Estado
      supabase.from("grupos").select("estado").eq("profesor_id", profesorId),
    ...
    if (instituciones.error) {
      if (DEBUG)
        console.error("Error al obtener instituciones:", instituciones.error);
      return NextResponse.json(
        {
          error: (await getApiTranslator(request, "dashboard")).t(
            "errors.fetchData"
    ...
    if (examenesRecientes.error) {
      if (DEBUG)
        console.error(
          "Error al obtener exámenes recientes:",
          examenesRecientes.error
        );
      return NextResponse.json(
        {
          error: (await getApiTranslator(request, "dashboard")).t(
    ...
    if (examenesCalificados.error) {
      if (DEBUG)
        console.error(
          "Error al obtener exámenes calificados:",
          examenesCalificados.error
        );
      return NextResponse.json(
        {
          error: (await getApiTranslator(request, "dashboard")).t(
    ...
    const gruposActivos =
      gruposData.data?.filter((g) => g.estado === "activo").length ?? 0;
    const gruposArchivados =
      gruposData.data?.filter((g) => g.estado === "archivado").length ?? 0;
    ...
    const tiempoAhorradoSegundos = examenesCalificadosTotal * 280;
    ...
    const examenesRecientesFormatted = await Promise.all(
      (examenesRecientes.data ?? []).map(async (examen) => {
        const materias = examen.materias as unknown as {
          nombre: string;
        } | null;

        // Obtener el primer grupo asignado a este examen desde examen_grupo
        const { data: aplicaciones, error: grupoError } = await supabase
          .from("examen_grupo")
        ...
        if (grupoError) {
          if (DEBUG)
            console.error(
              "Error al obtener grupo para examen:",
              examen.id,
              grupoError
            );
        }

        // Extraer todos los nombres de grupos
        const gruposNombres =
          aplicaciones?.map((app) => {
            const grupo = app.grupos as unknown as { nombre: string } | null;
