...
async function compressImage(
  ...
  try {
    ...
    if (buffer.length < 100) {
      if (DEBUG)
        logger.error("Buffer de imagen demasiado pequeño para comprimir");
      return buffer;
    }
    ...
    try {
      ...
      if (DEBUG) {
        logger.log(
          `Procesando imagen como WebP, ${
            isOriginal ? "con rotación" : "sin rotación"
          }`
        );
      }

      return await sharpInstance.toBuffer().catch((err) => {
  ...
  try {
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (
          fileAge > MAX_AGE ||
          (fileAge > MIN_IMAGE_AGE && file.match(/\.(jpg|jpeg|png|gif)$/i))
        ) {
          await _fsPromises.unlink(filePath);
          if (DEBUG) {
            logger.log(
              `Cleaned up old file: ${file}, age: ${
                fileAge / (60 * 60 * 1000)
              } hours`
            );
          }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const studentId =
      ...
      qrData.estudianteId ||
      qrData.student_id ||
      qrData.estudiante_id;
    const _groupId =
      qrData.groupId || qrData.grupoId || qrData.group_id || qrData.grupo_id;
    ...
    try {
      ...
      if (existingResultsError) {
        if (DEBUG) {
          logger.error(
            "Error al buscar resultados existentes:",
        ...
        if (DEBUG) {
          logger.log(
            `Encontrados ${existingResults.length} resultados existentes para eliminar`
          );
        }
        ...
        for (const result of existingResults) {
          ...
          if (escaneosError) {
            if (DEBUG) {
              logger.error(
                `Error al buscar escaneos para resultado ${resultadoId}:`,
                escaneosError
              );
            }
            ...
            if (DEBUG) {
              logger.log(
                `Encontrados ${escaneos.length} escaneos para eliminar`
              );
            }
            ...
            for (const escaneo of escaneos) {
              ...
              if (escaneo.ruta_s3_original) {
                ...
                const rutaRelativa = escaneo.ruta_s3_original.replace(
                  `${s3BucketName}/`,
                  ""
                );
                try {
                  ...
                  if (error) {
                    if (DEBUG) {
                      logger.error(
                        `Error al eliminar imagen original: ${error.message}`,
                        error
                      );
                    }
                  } else if (DEBUG) {
                    logger.log(
                      `Eliminada imagen original: ${rutaRelativa}`,
                      data
                    );
                  }
                } catch (e) {
              ...
              if (escaneo.ruta_s3_procesado) {
                // Extraer la ruta relativa sin el nombre del bucket
                const rutaRelativa = escaneo.ruta_s3_procesado.replace(
                  `${s3BucketName}/`,
                  ""
                );
                try {
                  ...
                  if (error) {
                    if (DEBUG) {
                      logger.error(
                        `Error al eliminar imagen procesada: ${error.message}`,
                        error
                      );
                    }
                  } else if (DEBUG) {
                    logger.log(
                      `Eliminada imagen procesada: ${rutaRelativa}`,
                      data
                    );
                  }
                } catch (e) {
              ...
              if (deleteEscaneoError) {
                if (DEBUG) {
                  logger.error(
                    `Error al eliminar escaneo ${escaneo.id}:`,
                    deleteEscaneoError
                  );
                }
              }
          ...
          if (deleteRespuestasError) {
            if (DEBUG) {
              logger.error(
                `Error al eliminar respuestas para resultado ${resultadoId}:`,
                deleteRespuestasError
              );
            }
          }
          ...
          if (deleteResultadoError) {
            if (DEBUG) {
              logger.error(
                `Error al eliminar resultado ${resultadoId}:`,
                deleteResultadoError
              );
            }
          }
        }
    ...
    if (isDuplicate && duplicateInfo && duplicateInfo.resultadoId) {
      try {
        if (DEBUG) {
          logger.log(
            "Eliminando resultado duplicado anterior:",
            duplicateInfo.resultadoId
          );
        }
        ...
        if (escaneoError) {
          if (DEBUG) {
            logger.error(
              "Error al obtener datos del escaneo anterior:",
              escaneoError
            );
          }
        }
        ...
        if (escaneoData) {
          ...
          if (escaneoData.ruta_s3_original) {
            try {
              ...
              if (error) {
                if (DEBUG) {
                  logger.error(
                    `Error al eliminar imagen original S3: ${error.message}`,
                    error
                  );
                }
              } else if (DEBUG) {
                logger.log(
                  `Eliminada imagen original: ${escaneoData.ruta_s3_original}`,
                  data
                );
              }
            } catch (e) {
            ...
            try {
              ...
              if (error) {
                if (DEBUG) {
                  logger.error(
                    `Error al eliminar imagen original S3 (ruta antigua): ${error.message}`,
                    error
                  );
                }
              } else if (DEBUG) {
                logger.log(
                  `Eliminada imagen original (ruta antigua): ${rutaAntigua}`,
                  data
                );
              }
            } catch (e) {
              if (DEBUG) {
                logger.error(
                  `Excepción al eliminar imagen original S3 (ruta antigua):`,
                  e
                );
              }
            }
          ...
          if (escaneoData.ruta_s3_procesado) {
            try {
              ...
              if (error) {
                if (DEBUG) {
                  logger.error(
                    `Error al eliminar imagen procesada S3: ${error.message}`,
                    error
                  );
                }
              } else if (DEBUG) {
                logger.log(
                  `Eliminada imagen procesada: ${escaneoData.ruta_s3_procesado}`,
                  data
                );
              }
            } catch (e) {
            ...
            try {
              ...
              if (error) {
                if (DEBUG) {
                  logger.error(
                    `Error al eliminar imagen procesada S3 (ruta antigua): ${error.message}`,
                    error
                  );
                }
              } else if (DEBUG) {
                logger.log(
                  `Eliminada imagen procesada (ruta antigua): ${rutaAntigua}`,
                  data
                );
              }
            } catch (e) {
              if (DEBUG) {
                logger.error(
                  `Excepción al eliminar imagen procesada S3 (ruta antigua):`,
                  e
                );
              }
            }
          ...
          if (deleteEscaneoError) {
            if (DEBUG) {
              logger.error(
                "Error al eliminar escaneo anterior:",
                deleteEscaneoError
              );
            }
          }
        ...
        if (deleteRespuestasError) {
          if (DEBUG) {
            logger.error(
              "Error al eliminar respuestas anteriores:",
              deleteRespuestasError
            );
          }
        }
        ...
        if (DEBUG) {
          logger.error(
            "Error durante la eliminación del resultado anterior:",
            error
          );
        }
      }
    ...
    const compressedProcessed = await compressImage(
      processedImage,
      80,
      800,
      false
    ...
    const originalFileName = `${uuidv4()}_original_${timestamp}.webp`;
    const processedFileName = `${uuidv4()}_processed_${timestamp}.webp`;
    ...
    const { data: resultadoExamen, error: resultadoExamenError } =
      await supabase
        .from("resultados_examen")
        .insert({
          examen_id: examId,
    ...
    if (resultadoExamenError) {
      if (DEBUG) {
        logger.error(
          "Error al guardar en resultados_examen:",
          resultadoExamenError
        );
      }
      return NextResponse.json(
    ...
    const respuestasToInsert = answers.map((respuesta: Answer) => ({
      ...
      puntaje_obtenido: respuesta.es_correcta
        ? parseFloat(
            preguntasHabilitadas.find(
              (p: Pregunta) => p.id === respuesta.pregunta_id
            )?.puntaje || "0"
          )
        : 0,
    }));
    ...
    if (respuestasError) {
      if (DEBUG) {
        logger.error(
          "Error al guardar respuestas individuales:",
          respuestasError
        );
      }
    ...
    try {
      ...
      if (processedImage && !processedImage.startsWith("data:")) {
        ...
        try {
          ...
          if (DEBUG) {
            logger.warn(
              `Could not delete processed image file ${processedPath}:`,
              err
            );
          }
        }
      ...
      if (originalImage && !originalImage.startsWith("data:")) {
        ...
        try {
          ...
          if (DEBUG) {
            logger.warn(
              `Could not delete original image file ${originalPath}:`,
              err
            );
          }
        }
